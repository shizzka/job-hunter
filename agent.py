...
async def do_search(dry_run: bool = False) -> dict:
    ...
    try:
        ...
        for v in all_vacancies:
            ...
            if source == "geekjob":
                await set_hunter_status("search_manual", "Ручной GJ", "busy")
                ...
                create_task(
                    ...
                    (
                        ...
                        f"{reason}\n"
                        f"URL: {v.get('url', '')}\n"
                        "Причина: автоотклик для GeekJob пока не подключён"
                    ),
                    "medium",
                )
                await notify_needs_manual(
                    ...
                    score,
                    reason,
                    note="GeekJob пока работает как manual-only источник.",
                )
                continue
