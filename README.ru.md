...
Поддерживает изолированные профили пользователей, LLM-анализ резюме, воронку откликов с A/B тестированием резюме, интерактивный мастер настройки, авто-ответ на анкеты работодателя (radio/checkbox/select), решение captcha hh.ru (vision-LLM + TG-bridge) и авто-диалог с AI-рекрутером в чатах hh.ru.
...
### AI-чаты на hh.ru
- Polling чатов на `chatik.hh.ru` каждые 30 минут (cron)
- Детект ботов hh.ru («ИИ-помощник», «Робот-помощник») по аватарке + alt-имени
- Авто-ответ через LLM с контекстом вакансии + фильтрованной базой знаний
- Safety: лимит 5 ответов на чат, cooldown между ответами, уведомление в TG на каждый отправленный ответ
...
# Чаты с AI-помощниками hh.ru
./run.sh chat-respond           # проверить чаты, ответить ИИ-помощникам
...
- `chat_responder_state.json` — last_replied_msg_id + replies_count per чат
...
- AI-чат-responder детектит только системных ботов hh.ru («ИИ-помощник», «Робот-помощник»). На живых HR-рекрутеров не отвечает (по дизайну, безопасности ради).
